...
class WC3MissionSlot(NamedTuple):
    campaign: WC3Campaign
    slot: Union[MissionPools, WC3Mission, None]


def create_regions(
    world: World, locations: Tuple[LocationData, ...], location_cache: List[Location]
) -> Tuple[Dict[WC3Campaign, Dict[str, MissionInfo]], int, str]:
    """
    Creates region connections by calling the multiworld's `connect()` methods
    Returns a 3-tuple containing:
    * dict[WC3Campaign, Dict[str, MissionInfo]] mapping a campaign and mission name to its data
...
def create_grid_regions(
    ...
    regions = [create_region(world, locations_per_region, location_cache, "Menu")]
    names: Dict[str, int] = {}
    missions: Dict[Tuple[int, int], WC3Mission] = {}
...
def create_structured_regions(
    ...
    if mission_order_type in campaign_depending_orders:
        ...
        for campaign in enabled_campaigns:
            ...
            for mission in mission_order[campaign]:
                ...
                if 0 < mission.removal_priority <= removals:
                    mission_slots.append(WC3MissionSlot(campaign, None))
                elif mission.type == MissionPools.FINAL:
                    if campaign == final_mission.campaign:
                        # Campaign is elected to be goal
                        mission_slots.append(WC3MissionSlot(campaign, final_mission))
                    else:
                        ...
                        mission_slots.append(WC3MissionSlot(campaign, campaign_difficulty))
                else:
                    mission_slots.append(WC3MissionSlot(campaign, mission.type))
    else:
        order = mission_order[WC3Campaign.GLOBAL]
        ...
        for mission in order:
            # Removing extra missions if mission pool is too small
            if 0 < mission.removal_priority <= removals:
                mission_slots.append(WC3MissionSlot(WC3Campaign.GLOBAL, None))
            elif mission.type == MissionPools.FINAL:
                mission_slots.append(WC3MissionSlot(WC3Campaign.GLOBAL, final_mission))
            else:
                mission_slots.append(WC3MissionSlot(WC3Campaign.GLOBAL, mission.type))
    ...
    for i in range(len(mission_slots)):
        ...
        if mission_slot is None:
            continue
        if isinstance(mission_slot, WC3MissionSlot):
    ...
    def pick_mission(slot):
        if shuffle_campaigns or mission_order_type not in campaign_depending_orders:
            ...
            mission_slots[slot] = WC3MissionSlot(slot_campaign, mission)
        else:
    ...
    missions_to_add: List[WC3Mission] = mission_pools[MissionPools.STARTER]
    ...
    for mission_slot in mission_slots:
        if isinstance(mission_slot.slot, WC3Mission):
    ...
    mission_req_table: Dict[WC3Campaign, Dict[str, MissionInfo]] = {}
    campaign_mission_slots: Dict[WC3Campaign, List[WC3MissionSlot]] = \
        {
            campaign: [mission_slot for mission_slot in mission_slots if campaign == mission_slot.campaign]
            for campaign in campaigns
        }

    slot_map: Dict[WC3Campaign, List[int]] = dict()
    ...
    for campaign in campaigns:
        ...
        for i, mission in enumerate(campaign_mission_slots[campaign]):
            if mission is None or mission.slot is None:
                continue
            connections: List[MissionConnection] = []
            all_connections: List[WC3MissionSlot] = []
            ...
            for connection in mission_order[campaign][i].connect_to:
                if connection.connect_to == -1:
                    ...
                    if isinstance(required_mission.slot, WC3Mission):
